...
public class RendezVous extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
       ...
       // List<RDV> rdvs = rDVFacade.findByDate(Date.from(dateSelected.atStartOfDay().atZone(ZoneId.systemDefault()).toInstant()));
        ...
        int found = 0; String serviceName = "";
        for (RDV rdv : rdvs) {
        ...
        if (found >= 9) {
            request.setAttribute("error max rdv", " le service "+serviceName+" a eu un maximum de rendez-vous pour le jour "+dateChoisi);
        ...
        for (RDV rdv : findByDateAndPatient) {
            if (rdv.getService().getId().longValue() == Long.valueOf(serviceID).longValue()) {
                request.setAttribute("error max rdv by service", " vous avez deja un rendez-vous pour le service "+serviceName+" le : "+dateSelected.toString());
            request.getRequestDispatcher("rendezvous.jsp").forward(request, response);
             return;
            }
        }
        ...
        if (foundRdv>=3) {
            ...
            request.setAttribute("error max rdv by service", " Vous avez attein la limite de 3 rendez vous par 14 jour: prochain rendez vous possible pour "+LocalDate.now().plusDays(14).toString());
            request.getRequestDispatcher("rendezvous.jsp").forward(request, response);
             return;
        }
